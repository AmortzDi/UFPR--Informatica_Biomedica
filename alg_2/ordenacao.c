...
struct pilha* cria_pilha(int tamanho) {
    ...
    if (!(pilha = malloc(sizeof(struct pilha)))) {
        printf("ERRO PILHA");
        return NULL;
    }

    if (!(pilha->vetor_indices = malloc(tamanho * sizeof(int)))) {
...
void empilha(struct pilha *pilha, int indice) {
    if (pilha->indice_do_topo == pilha->tamanho_pilha - 1) {
...
void merge_sort(int vetor[], int a, int b, uint64_t *numComparacoes) {
    struct pilha *pilha;
    int m;

    pilha = cria_pilha(b - a + 1);

    // Empilha os limites inicial e final do vetor
    empilha(pilha, b);
    empilha(pilha, a);
    ...
    while (!pilha_vazia(pilha)) {
        ...
        // Se os limites forem válidos para um subvetor
        if (b > a) {
            m = a + (b - a) / 2; 

            // Empilha os limites dos subvetores esquerdo e direito
            empilha(pilha, b);
            empilha(pilha, m + 1);
            ...
            // Chama a função merge para mesclar os subvetores ordenados
...
uint64_t mergeSortSR(int vetor[], size_t tam) {
    uint64_t numComparacoes = 0;
    if (tam > 0)
        merge_sort(vetor, 0, tam - 1, &numComparacoes);
    return numComparacoes;
}
